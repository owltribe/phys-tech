...
from src.service_request.schemas import ServiceRequestRead, ServiceRequestFilter, ServiceRequestCreate, \
...
def paginated_list(service_request_filter: ServiceRequestFilter = FilterDepends(ServiceRequestFilter)):
    return service.list(service_request_filter)


@service_request_router.post("", response_model=ServiceRequestRead)
def create(service_request_create: ServiceRequestCreate, user: User = Depends(current_active_user)):
...
@service_request_router.delete("/{service_request_id}", response_model=None, status_code=status.HTTP_204_NO_CONTENT)
